...
const App: React.FC = () => {
  ...
  const handleAudioPlayer = async({ audioName }: { audioName: string }) => {
  ...
  const handleExportToAudio = useCallback(async(filename: string) => {
    try {
        ...
        // Aca tengo que llamar al backend y no directamente a la api de google
        // const response = await fetch('http://localhost:5000/api/generar_audio?filename=procesado_' + filename);

        // ENVIAR filename COMO PARÁMETRO, NO USAR VARIABLE GLOBAL
        ...
        if (response.ok) {            
            // Verifico si el audio existe en la carpeta app/shared-files/audio
            const audioName = `procesado_${filename.split('.').slice(0, -1).join('.')}`; // Nombre sin extensión
            handleAudioPlayer({ audioName });
        } else {
        ...
        >
          {!file ? (
            ...
            <div className="space-y-6">
              ...
              {/* 
                aca debería aparecer un reproductor de audio si el audio fue generado
                y un boton para descargar el archivo de texto procesado
              */}
